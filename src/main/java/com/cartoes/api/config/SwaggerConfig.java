...
public class SwaggerConfig {
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.cartoes.api.controllers")).paths(PathSelectors.any())
				.build().apiInfo(apiInfo());
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Aplicação de cartões de crédito API")
				.description("Documentação da API de cartões de crédito.").version("1.0").build();
	}
